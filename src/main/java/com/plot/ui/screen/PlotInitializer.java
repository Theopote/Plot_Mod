...
public class PlotInitializer {
    private static final Logger LOGGER = LoggerFactory.getLogger("Plot/PlotInitializer");
    ...
    public static void initialize() {
        LOGGER.debug("PlotInitializer.initialize() 被调用 (initialized: {}, initializing: {})", 
    ...
    private static void scheduleRetryIfNeeded(MinecraftClient client) {
        synchronized (INIT_LOCK) {
            if (retryCount < MAX_RETRY_ATTEMPTS) {
                ...
                }, "Plot-Init-Retry-" + retryCount).start();
            } else {

...
public class ExtendHandler implements IModifyHandler {
    ...
    private Vec2d calculateExtendDirection(Shape shape, Vec2d extendPoint) {
        try {
            ...
            // 对于多段线，需要特殊处理
            if (shape instanceof com.plot.core.geometry.shapes.PolylineShape) {
                return calculatePolylineExtendDirection((com.plot.core.geometry.shapes.PolylineShape) shape, extendPoint);
            }
            
            // 对于圆弧，需要特殊处理
            if (shape instanceof com.plot.core.geometry.shapes.ArcShape) {
                return calculateArcExtendDirection((com.plot.core.geometry.shapes.ArcShape) shape, extendPoint);
            }
     ...
     * 计算多段线的延伸方向
     */
    private Vec2d calculatePolylineExtendDirection(com.plot.core.geometry.shapes.PolylineShape polyline, Vec2d extendPoint) {
        try {
     ...
     */
    private Vec2d calculateArcExtendDirection(com.plot.core.geometry.shapes.ArcShape arc, Vec2d extendPoint) {
        try {
            // 获取圆弧的端点
    ...
    private Vec2d findExtendTarget(Shape shape, Vec2d startPoint, Vec2d direction, 
        ...
        try {
            ...
            com.plot.core.spatial.SpatialIndex spatialIndex = getSpatialIndex();
    ...
    private Vec2d findProjectionIntersection(Shape shape, Shape boundary, Vec2d startPoint, Vec2d direction) {
        ...
        switch (boundary) {
            ...
            case com.plot.core.geometry.shapes.CircleShape circleBoundary -> {
                return findCircleProjectionIntersection(startPoint, rayEnd, circleBoundary);
            }
            case com.plot.core.geometry.shapes.ArcShape arcBoundary -> {
                return findArcProjectionIntersection(startPoint, rayEnd, arcBoundary);
            }
            case com.plot.core.geometry.shapes.PolylineShape polylineBoundary -> {
                return findPolylineProjectionIntersection(startPoint, rayEnd, polylineBoundary);
            }
            case com.plot.core.geometry.shapes.EllipseShape ellipseBoundary -> {
     ...
     */
    private Vec2d findPolylineProjectionIntersection(Vec2d rayStart, Vec2d rayEnd, com.plot.core.geometry.shapes.PolylineShape polyline) {
        try {
            List<Vec2d> points = polyline.getPoints();
            if (points == null || points.size() < 2) {
                return null;
     ...
     * 查找与椭圆的投影交点
     */
    private Vec2d findEllipseProjectionIntersection(Vec2d rayStart, Vec2d rayEnd, com.plot.core.geometry.shapes.EllipseShape ellipse) {
        try {
            // 对于椭圆，使用简化的圆形近似方法
            ...
            // 创建临时圆形进行交点计算
            com.plot.core.geometry.shapes.CircleShape tempCircle = 
                new com.plot.core.geometry.shapes.CircleShape(center, radius);
     ...
     * 查找与直线的投影交点（延伸到延长线位置）
     */
    private Vec2d findLineProjectionIntersection(Vec2d rayStart, Vec2d rayEnd, com.plot.core.geometry.shapes.LineShape lineBoundary) {
     ...
     * 查找与圆的投影交点
     */
    private Vec2d findCircleProjectionIntersection(Vec2d rayStart, Vec2d rayEnd, com.plot.core.geometry.shapes.CircleShape circleBoundary) {
     ...
     * 查找与圆弧的投影交点（延伸到完整圆位置）
     */
    private Vec2d findArcProjectionIntersection(Vec2d rayStart, Vec2d rayEnd, com.plot.core.geometry.shapes.ArcShape arcBoundary) {
        // 在投影模式下，计算与完整圆的交点，即使不在圆弧范围内也返回
        // 这样可以延伸到边界图形的延长线位置（虽然看不见，但默认边界图形长度无限）
        com.plot.core.geometry.shapes.CircleShape circle = new com.plot.core.geometry.shapes.CircleShape(
     ...
     * 检查点是否在圆弧的角度范围内
     */
    private boolean isPointInArcRange(Vec2d point, com.plot.core.geometry.shapes.ArcShape arc) {

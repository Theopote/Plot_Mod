...
public class SelectionStrategy implements IModifyStrategy {
    ...
    // 控制点编辑策略缓存
    private com.plot.ui.tools.impl.modify.strategy.ControlPointEditStrategy cachedEditStrategy = null;
    ...
    public ModifyResult onMouseDown(Vec2d pos, int button, ModifyToolContext context) {
        ...
        if (editTool != null && editTool.isActive()) {
            ...
            if (cachedEditStrategy == null) {
                cachedEditStrategy = new com.plot.ui.tools.impl.modify.strategy.ControlPointEditStrategy(editTool);
            }
    ...
    private void activateControlPointEdit(Shape shape, ModifyToolContext context) {
        try {
            // 获取控制点编辑工具
            com.plot.ui.tools.impl.modify.ControlPointEditTool editTool = 
    ...
    private boolean isShapeFullyInsideLasso(Shape shape, List<Vec2d> lasso) {
        try {
            // 规则：完全包含 + 不与套索边界相交
            // 1) 如与套索边界相交，则不是完全包含
            if (com.plot.ui.tools.impl.modify.helper.GeometricSelectionHelper.isLassoIntersectsShape(shape, lasso)) {
                return false;
            }
            ...
            for (Vec2d p : reps) {
                if (!com.plot.ui.tools.impl.modify.helper.GeometricSelectionHelper.isPointInPolygon(p, lasso)) {
                    return false;
                }
            }
            return true;

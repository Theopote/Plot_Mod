...
public class PropertyPanel implements UIComponent {
    ...
    public void init() {
        if (initialized) {
            PlotMod.LOGGER.debug("PropertyPanel已经初始化，跳过初始化流程");
            return;
        }

        try {
            PlotMod.LOGGER.info("正在初始化PropertyPanel...");
            ...
            initialized = true;
            PlotMod.LOGGER.info("PropertyPanel初始化完成");
        } catch (Exception e) {
            PlotMod.LOGGER.error("PropertyPanel初始化失败: {}", e.getMessage(), e);
        }
    }
    ...
    public void render() {
        if (!initialized) {
            PlotMod.LOGGER.debug("PropertyPanel未初始化，跳过渲染");
            return;
        }
        ...
        try {
            ...
            PlotMod.LOGGER.error("PropertyPanel渲染失败: {}", e.getMessage(), e);
        }
    }
    ...
    public void close() throws Exception {
        try {
            PlotMod.LOGGER.debug("正在关闭PropertyPanel...");

...
public class AlignToolOptionRenderer extends AbstractToolOptionRenderer {
    ...
    public float render() {
        ...
        try {
            PlotMod.LOGGER.debug("AlignToolOptionRenderer: 开始渲染对齐工具选项");
            
            // === 缩放设置 ===
            height += renderScaleSettings();
            
        } catch (Exception e) {
            PlotMod.LOGGER.error("渲染对齐工具选项失败: {}", e.getMessage(), e);
        } finally {
            ImGui.popID();
        }
    ...
    private float renderScaleSettings() {
        ...
        var theme = com.plot.ui.theme.ThemeManager.getInstance().getCurrentTheme();
    ...
    private void syncToolState() {
        try {
            ...
            PlotMod.LOGGER.warn("同步对齐工具状态失败: {}", e.getMessage());
        }
    }
    ...
    private AlignTool getCurrentTool() {
        ...
        try {
            ...
            PlotMod.LOGGER.warn("获取当前对齐工具失败: {}", e.getMessage());
        }
        return null;
    }
    ...
    public void cleanup() {
        // 清理资源
        alignTool = null;
        PlotMod.LOGGER.debug("AlignToolOptionRenderer 清理完成");
    }
} 
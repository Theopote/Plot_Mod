...
public class SelectionToolOptionRenderer extends AbstractToolOptionRenderer {
    ...
    public SelectionToolOptionRenderer() {
        ...
        PlotMod.LOGGER.info("创建选择工具选项渲染器");
        
        // 加载图标
        this.normalSelectIconId = ImGuiUtils.getTextureId(
            Identifier.of("plot", "textures/gui/tooloptionspanel/select_normal.png"));
        this.lassoSelectIconId = ImGuiUtils.getTextureId(
            Identifier.of("plot", "textures/gui/tooloptionspanel/select_lasso.png"));
        
        PlotMod.LOGGER.info("选择工具选项渲染器初始化完成，图标ID: normal={}, lasso={}", 
    ...
    public float render() {
        ...
        try {
            ...
            try {
                ...
                PlotMod.LOGGER.error("获取选择模式失败", e);
                selectionMode = SelectionTool.CONFIG_VALUE_NORMAL;
            }
            ...
            if (clicked && !isNormalMode) {
                PlotMod.LOGGER.info("SelectionToolOptionRenderer: 用户点击普通选择按钮");
                selectionMode = SelectionTool.CONFIG_VALUE_NORMAL;
            ...
            if (clicked && !isLassoMode) {
                PlotMod.LOGGER.info("SelectionToolOptionRenderer: 用户点击套索选择按钮");
            ...
            PlotMod.LOGGER.debug("选择工具选项实际渲染高度: {} (从 {} 到 {})", height, startY, endY);
            
        } catch (Exception e) {
            PlotMod.LOGGER.error("渲染选择工具选项时发生异常", e);
        } finally {
            ImGui.popID();
        }

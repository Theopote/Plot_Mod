...
public final class SingleTowerGhostPreview {
    ...
    public static void projectToGhosts(PluginContext host, PowerLineGenerationResult result) {
        if (host == null || result == null) {
            return;
        }
        com.plot.api.world.IGhostBlockService ghosts = host.ghosts();
        if (ghosts == null) {
            return;
        }
        ghosts.clearAllGhostBlocks();
        Map<net.minecraft.util.math.BlockPos, String> blocks = new LinkedHashMap<>();

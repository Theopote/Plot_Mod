package com.plot.plugin.powerline.engineering;

/** 稳定规则 ID 常量。 */
public final class EngineeringRuleIds {
    public static final String CLEARANCE_GROUND_MINIMUM = "clearance.ground.minimum";
    public static final String SPAN_MAXIMUM = "span.maximum";
    public static final String SPAN_MINIMUM = "span.minimum";
    public static final String TOWER_ROLE_ANGLE = "tower.role.angle";
    public static final String CONDUCTOR_SEPARATION_PHASE = "conductor.separation.phase";
    public static final String CONDUCTOR_SEPARATION_GROUND = "conductor.separation.ground";
    public static final String SAG_MAXIMUM = "sag.maximum";
    public static final String TOWER_BASE_UNEVEN = "tower.base.uneven";
    public static final String TOWER_HEIGHT_MARGIN = "tower.height.margin";

    private EngineeringRuleIds() {
    }
}

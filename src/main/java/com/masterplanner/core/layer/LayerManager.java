    ...
    public void updateLayerProperty(ILayer layer, String propertyType, Object newValue) {
        ...
        try {
            switch (propertyType.toLowerCase()) {
                ...
                case "linestyle":
                case "line_style":
                    if (newValue instanceof com.masterplanner.core.graphics.style.LineStyle) {
                        // 先更新图层的线条样式
                        layer.setLineStyle((com.masterplanner.core.graphics.style.LineStyle) newValue);
                        
                        // 如果是Layer类型，强制触发图层上所有图形的样式更新
                        if (layer instanceof com.masterplanner.core.layer.Layer concreteLayer) {

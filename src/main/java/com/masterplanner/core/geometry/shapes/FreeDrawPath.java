    ...
    public List<Shape> breakShape(Vec2d firstBreakPoint, Vec2d secondBreakPoint, String breakMode) {
        ...
        try {
            if ("SINGLE_POINT".equals(breakMode)) {
                // 单点打断：在打断点处分割路径
                int segmentIndex = com.masterplanner.core.geometry.GeometryUtils.findSegmentContainingPoint(pathPoints, localFirstPoint);
                if (segmentIndex >= 0) {
                    // 精确投影断点到该线段上
                ...
                // 两点打断：移除两点之间的部分
                int firstSegment = com.masterplanner.core.geometry.GeometryUtils.findSegmentContainingPoint(pathPoints, localFirstPoint);
                int secondSegment = com.masterplanner.core.geometry.GeometryUtils.findSegmentContainingPoint(pathPoints, localSecondPoint);

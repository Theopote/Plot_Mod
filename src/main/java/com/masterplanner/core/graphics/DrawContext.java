...
import static com.masterplanner.core.graphics.DrawingConstants.*;
...
public class DrawContext {
    ...
    public void setFont(Font font) {
        if (font != null) {
            ...
            this.currentFontFamily = font.getFamily();
            this.currentFontSize = font.getSize();
            this.currentFontBold = font.isBold();
            this.currentFontItalic = font.isItalic();
            
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug("设置字体: {}", font);
            }
        }
    }
     ...
     * 绘制自定义线型
     */
    private void drawCustomLine(Vec2d start, Vec2d end, LineStyle style) {
        double[] pattern = getCustomPatternSafely(style);
        if (pattern == null || pattern.length == 0) {
            LOGGER.warn("自定义线型模式为空，使用实线");
            drawLine(start, end, color);
            return;
        }
        ...
        // 根据自定义模式绘制线段
        List<Vec2d> linePoints = calculateCustomLinePoints(screenStart, direction, length, pattern);
        
        // 批量绘制所有线段
        for (int i = 0; i < linePoints.size() - 1; i += 2) {
            Vec2d segStart = linePoints.get(i);
            Vec2d segEnd = linePoints.get(i + 1);
     ...
     * 安全地获取自定义线型模式
     * @param style 线型样式
     * @return 自定义线型模式数组，如果不可用则返回默认模式
     */
    private double[] getCustomPatternSafely(LineStyle style) {
        if (style == null) return null;
        
        // 直接调用getCustomPattern方法获取自定义线型模式
        double[] pattern = style.getCustomPattern();
        
        // 如果没有自定义模式，返回默认的虚线模式
        if (pattern == null || pattern.length == 0) {
            return new double[] { DASH_LENGTH, GAP_LENGTH };
        }
     ...
     * 计算自定义线型的点
     */
    private List<Vec2d> calculateCustomLinePoints(Vec2d start, Vec2d direction, double length, double[] pattern) {
        List<Vec2d> points = new ArrayList<>();
        double currentLength = 0;
        int patternIndex = 0;

        while (currentLength < length) {
            double segmentLength = pattern[patternIndex];
            double endPos = Math.min(currentLength + segmentLength, length);
            
            points.add(start.add(direction.multiply(currentLength)));
            points.add(start.add(direction.multiply(endPos)));
     ...
     * 绘制点线
     */
    private void drawDottedLine(Vec2d start, Vec2d end, Color color) {
        Vec2d screenStart = worldToScreen(start);
        Vec2d screenEnd = worldToScreen(end);
        ...
        // 计算点线的点
        List<Vec2d> dotPoints = calculateDotPoints(screenStart, direction, length);
        
        // 绘制所有点
        for (Vec2d point : dotPoints) {
            drawList.addCircleFilled(
     ...
     * 绘制点划线
     */
    private void drawDashDotLine(Vec2d start, Vec2d end, Color color) {
        Vec2d screenStart = worldToScreen(start);
        Vec2d screenEnd = worldToScreen(end);
        ...
        // 计算点划线的点
        List<Vec2d> dashDotPoints = calculateDashDotPoints(screenStart, direction, length);
        
        // 绘制线段和点
        for (int i = 0; i < dashDotPoints.size() - 1; i += 2) {
            Vec2d segStart = dashDotPoints.get(i);
            Vec2d segEnd = dashDotPoints.get(i + 1);
        ...
        // 绘制点
        for (int i = 2; i < dashDotPoints.size(); i += 4) {
            Vec2d point = dashDotPoints.get(i);
            drawList.addCircleFilled(
                (float)point.x, (float)point.y,
                lineWidth / 2, imColor
            );
     ...
     * 计算点线的点
     */
    private List<Vec2d> calculateDotPoints(Vec2d start, Vec2d direction, double length) {
        List<Vec2d> points = new ArrayList<>();
        double currentLength = 0;
        ...
        while (currentLength < length) {
            ...
            // 跳过间隔
            currentLength += DOT_LENGTH + GAP_LENGTH;
        }
     ...
     * 计算点划线的点
     */
    private List<Vec2d> calculateDashDotPoints(Vec2d start, Vec2d direction, double length) {
        List<Vec2d> points = new ArrayList<>();
        double currentLength = 0;
        ...
        while (currentLength < length) {
            ...
            if (currentLength < length) {
                points.add(start.add(direction.multiply(currentLength)));
                currentLength += DOT_LENGTH + DOT_GAP_LENGTH;
            }
        }
     ...
     * 计算双点划线的点
     */
    private List<Vec2d> calculateDashDotDotPoints(Vec2d start, Vec2d direction, double length) {
        List<Vec2d> points = new ArrayList<>();
        double currentLength = 0;

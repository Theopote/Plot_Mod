...
import com.masterplanner.api.tool.*;
import com.masterplanner.api.event.IEvent;
...
public class ToolManager implements IToolManager {
    ...
    private void setupToolCanvasAndCamera(ITool tool) {
        ...
        try {
            // 获取Canvas
            com.masterplanner.api.model.ICanvas canvas = getCanvas();
            if (canvas == null) {
            ...
            if (tool instanceof com.masterplanner.ui.tools.impl.drawing.DrawingTool drawingTool) {

                // 设置Canvas
                if (canvas instanceof com.masterplanner.ui.canvas.Canvas uiCanvas) {
                    drawingTool.setCanvas(uiCanvas);
                    
                    // 设置Camera
                    com.masterplanner.ui.canvas.CanvasCamera camera = uiCanvas.getCamera();
                    if (camera != null) {
            ...
            if (tool instanceof com.masterplanner.ui.tools.impl.modify.ModifyTool modifyTool) {
                if (canvas instanceof com.masterplanner.ui.canvas.Canvas uiCanvas) {
     ...
     * @param event 选择变更事件
     */
    private void onSelectionChanged(com.masterplanner.infrastructure.event.base.Event event) {

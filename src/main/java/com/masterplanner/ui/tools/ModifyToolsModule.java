package com.masterplanner.ui.tools;
...
import com.masterplanner.core.tool.ToolGroupImpl;
import com.masterplanner.infrastructure.event.EventBus;
import com.masterplanner.ui.tools.impl.modify.*;
...
public final class ModifyToolsModule {
    ...
    private static List<BaseTool> createAllModifyTools(
        ...
        try {
            ...
            com.masterplanner.api.model.ICanvas canvas = appState.getCanvas();
            if (canvas == null) {
    ...
    private static void registerTools(ToolManager toolManager, ToolGroup modifyGroup, List<BaseTool> tools) {
        ...
        for (BaseTool tool : tools) {
            try {
                ...
                // 激进策略：立即抛出异常，中断初始化过程
                String errorMsg = String.format("注册修改工具 '%s' 失败，应用无法正常启动", tool.getName());
                LOGGER.error(errorMsg, e);
                throw new RuntimeException(errorMsg, e);
            }
        }

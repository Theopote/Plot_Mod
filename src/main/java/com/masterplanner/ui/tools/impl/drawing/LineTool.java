...
public class LineTool extends DrawingTool {
    ...
    private static class MultiLineShape extends Shape {
        ...
        @Override
        public com.masterplanner.core.geometry.BoundingBox getBoundingBox() {
            if (lines.isEmpty()) return null;
            
            com.masterplanner.core.geometry.BoundingBox bounds = lines.getFirst().getBoundingBox();
            for (int i = 1; i < lines.size(); i++) {
                com.masterplanner.core.geometry.BoundingBox lineBounds = lines.get(i).getBoundingBox();
                if (lineBounds != null) {

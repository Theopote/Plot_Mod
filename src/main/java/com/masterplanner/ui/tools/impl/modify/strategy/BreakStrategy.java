...
public class BreakStrategy implements IModifyStrategy {
    ...
    private void createBreakParameters(ModifyToolContext context) {
        ...
        try {
            // 传递具体 AppState（处理器需要 com.masterplanner.core.state.AppState）
            if (context != null) {
                // 新增：传递上下文给处理器，用于错误反馈
                breakParameters.setParameter("context", context);
                
                Object apiState = context.getAppState();
                if (apiState instanceof com.masterplanner.core.state.AppState concrete) {
                    breakParameters.setParameter("appState", concrete);
                } else {
                    // 兜底：尝试全局实例
                    try {
                        com.masterplanner.core.state.AppState concrete = com.masterplanner.core.state.AppState.getInstance();
                        if (concrete != null) {
                            breakParameters.setParameter("appState", concrete);
                        }
                    } catch (Throwable t) {
    ...
    private ModifyCommand createBreakCommand(ModifyToolContext context) {
        try {
            ...
            com.masterplanner.ui.tools.impl.modify.adapter.ModifyAdapter adapter = 
                com.masterplanner.ui.tools.impl.modify.adapter.ModifyAdapter.getInstance();
    ...
    private Vec2d projectPointOnComplexShape(Vec2d pos) {
        try {
            ...
            int segmentIndex = com.masterplanner.core.geometry.GeometryUtils.findSegmentContainingPoint(points, localPos);
            if (segmentIndex >= 0) {

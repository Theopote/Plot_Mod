...
public class CircleTool extends DrawingTool {
    ...
    private com.masterplanner.core.snap.SnapPriorityEvaluator.SnapType currentSnapType = 
        com.masterplanner.core.snap.SnapPriorityEvaluator.SnapType.NONE; // 当前捕捉类型
    ...
    private void renderSnapIndicator(ImDrawList drawList, CanvasCamera camera) {
        ...
        // 根据捕捉类型使用统一样式
        Color indicatorColor = com.masterplanner.ui.tools.snap.SnapVisualStyle.colorFor(currentSnapType);
        float indicatorSize = com.masterplanner.ui.tools.snap.SnapVisualStyle.ringSizeFor(currentSnapType);
    ...
    private void renderSnapTypeIndicatorImGui(ImDrawList drawList, CanvasCamera camera, Vec2d point, 
        ...
        switch (snapType) {
            case END_POINT -> {
                // 端点：实心正方形
                int color = com.masterplanner.ui.tools.snap.SnapVisualStyle.imGuiColorFor(com.masterplanner.core.snap.SnapPriorityEvaluator.SnapType.END_POINT, 255);
                drawList.addRectFilled(x - size, y - size, x + size, y + size, color);
            }
            case MID_POINT -> {
                // 中点：三角形标记
                int color = com.masterplanner.ui.tools.snap.SnapVisualStyle.imGuiColorFor(com.masterplanner.core.snap.SnapPriorityEvaluator.SnapType.MID_POINT, 255);
                drawList.addTriangleFilled(x, y - size, x - size, y + size, x + size, y + size, color);
            }
            case CENTER_POINT, CENTROID -> {
                // 中心点：十字标记
                int color = com.masterplanner.ui.tools.snap.SnapVisualStyle.imGuiColorFor(com.masterplanner.core.snap.SnapPriorityEvaluator.SnapType.CENTER_POINT, 255);
                drawList.addLine(x - size, y, x + size, y, color, 2.0f);
                drawList.addLine(x, y - size, x, y + size, color, 2.0f);
            }
            case INTERSECTION, VERTEX -> {
                // 交点：X标记
                int color = com.masterplanner.ui.tools.snap.SnapVisualStyle.imGuiColorFor(com.masterplanner.core.snap.SnapPriorityEvaluator.SnapType.INTERSECTION, 255);
                drawList.addLine(x - size, y - size, x + size, y + size, color, 2.0f);
                drawList.addLine(x - size, y + size, x + size, y - size, color, 2.0f);
            }
            case PERPENDICULAR, HORIZONTAL, VERTICAL -> {
                // 垂足：垂直线标记
                int color = com.masterplanner.ui.tools.snap.SnapVisualStyle.imGuiColorFor(com.masterplanner.core.snap.SnapPriorityEvaluator.SnapType.PERPENDICULAR, 255);
                drawList.addLine(x, y - size, x, y + size, color, 2.0f);
                drawList.addLine(x - size * 0.5f, y + size * 0.5f, x + size * 0.5f, y + size * 0.5f, color, 2.0f);
            }
            case TANGENT -> {
                // 切点：小圆标记
                int color = com.masterplanner.ui.tools.snap.SnapVisualStyle.imGuiColorFor(com.masterplanner.core.snap.SnapPriorityEvaluator.SnapType.TANGENT, 255);
    ...
    private Color getSnapIndicatorColor(com.masterplanner.core.snap.SnapPriorityEvaluator.SnapType snapType) {
        return com.masterplanner.ui.tools.snap.SnapVisualStyle.colorFor(snapType);
    }
    ...
    private float getSnapIndicatorSize(com.masterplanner.core.snap.SnapPriorityEvaluator.SnapType snapType) {
        ...
        float base = com.masterplanner.ui.tools.snap.SnapVisualStyle.ringSizeFor(snapType);
        // 保持与原有常量的相对一致性：按比例微调
        float scale = SNAP_INDICATOR_SIZE / com.masterplanner.ui.tools.snap.SnapVisualStyle.DEFAULT_RING_SIZE;
        return base * scale;
    }
     ...
     * 支持三种绘制模式的多点点击交互
     */
    private class CircleInteractionStrategy implements com.masterplanner.ui.tools.impl.drawing.strategy.IInteractionStrategy {
        ...
        public com.masterplanner.core.model.Shape getFinalShape() {
            if (previewCircle != null) {
                ...
                // 应用样式
                com.masterplanner.core.graphics.style.ShapeStyle style = getStyleHandler().getFinalStyle();
                if (style != null) {
                    circle.setStyle(style);
                }

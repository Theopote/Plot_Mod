...
public class GeometricSelectionHelper {
    ...
    private static boolean isShapeCompletelyInRectangle(Shape shape, double minX, double minY, double maxX, double maxY) {
        ...
        if (shape instanceof com.masterplanner.core.geometry.shapes.BezierCurveShape) {
            try {
                List<Vec2d> anchorPoints = ((com.masterplanner.core.geometry.shapes.BezierCurveShape) shape).getAnchorPoints();
    ...
    private static boolean useGeometricRectangleIntersection(Shape shape, double minX, double minY, 
        ...
        if (shape instanceof com.masterplanner.core.geometry.shapes.BezierCurveShape) {
            return isBezierCurveIntersectsRectangle((com.masterplanner.core.geometry.shapes.BezierCurveShape) shape, minX, minY, maxX, maxY);
        }
     ...
     * 检查贝塞尔曲线是否与矩形相交
     */
    private static boolean isBezierCurveIntersectsRectangle(com.masterplanner.core.geometry.shapes.BezierCurveShape curve, double minX, double minY, double maxX, double maxY) {
        // 方法1：检查曲线是否完全包含在矩形内
        ...
        if (curvePoints != null && curvePoints.size() >= 2) {
            // 创建矩形的四条边
            com.masterplanner.core.geometry.shapes.LineShape[] edges = {
                new com.masterplanner.core.geometry.shapes.LineShape(new Vec2d(minX, minY), new Vec2d(maxX, minY)), // 下边
                new com.masterplanner.core.geometry.shapes.LineShape(new Vec2d(maxX, minY), new Vec2d(maxX, maxY)), // 右边
                new com.masterplanner.core.geometry.shapes.LineShape(new Vec2d(maxX, maxY), new Vec2d(minX, maxY)), // 上边
                new com.masterplanner.core.geometry.shapes.LineShape(new Vec2d(minX, maxY), new Vec2d(minX, minY))  // 左边
            };
            ...
            for (int i = 0; i < curvePoints.size() - 1; i++) {
                ...
                for (com.masterplanner.core.geometry.shapes.LineShape edge : edges) {
                    if (doLineSegmentsIntersect(p1, p2, edge.getStart(), edge.getEnd())) {

...
public class MirrorStrategy implements IModifyStrategy {
    ...
    private List<Vec2d> getShapeKeyPoints(Shape shape) {
        ...
        // 根据图形类型添加特殊点
        switch (shape) {
            case com.masterplanner.core.geometry.shapes.CircleShape circleShape ->
                // 圆形：添加中心点
                    keyPoints.add(circleShape.getCenter());
            case com.masterplanner.core.geometry.shapes.RectangleShape rectangleShape -> {
                // 矩形：添加四个角点
                List<Vec2d> corners = getRectangleCorners(rectangleShape);
                keyPoints.addAll(corners);
            }
            case com.masterplanner.core.geometry.shapes.EllipseShape ellipseShape ->
                // 椭圆：添加中心点
                    keyPoints.add(ellipseShape.getCenter());
            case com.masterplanner.core.geometry.shapes.LineShape lineShape -> {
                // 直线：添加起点、终点、中点
     ...
     * 获取矩形的四个角点
     */
    private List<Vec2d> getRectangleCorners(com.masterplanner.core.geometry.shapes.RectangleShape rectangle) {

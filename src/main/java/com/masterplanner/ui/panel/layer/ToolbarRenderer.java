...
public class ToolbarRenderer {
    ...
    public void render(float width) {
        ...
        try {
            // 获取当前主题颜色
            com.masterplanner.ui.theme.UITheme.ThemeColors currentTheme = 
                com.masterplanner.ui.theme.ThemeManager.getInstance().getCurrentTheme();
    ...
    private void selectAllElementsInActiveLayer() {
        ...
        if (activeLayer instanceof Layer layer) {
            // 发送选择图层所有图元的事件
            eventBus.publish(new com.masterplanner.core.layer.LayerEventSystem.SelectAllElementsInLayerEvent(layer.getId(), layer));
        }
    }
}
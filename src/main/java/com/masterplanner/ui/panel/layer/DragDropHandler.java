...
public class DragDropHandler {
    ...
    private void handleLayerMove(Layer layer, int targetIndex) {
        try {
            ...
            MasterPlannerMod.LOGGER.debug("图层移动: {} -> 位置 {}",
                layer.getName(), targetIndex);
        } catch (Exception e) {
            MasterPlannerMod.LOGGER.error("图层移动失败: {}", e.getMessage(), e);
    ...
    private int calculateDropIndex(float mouseY) {
        try {
            ...
            MasterPlannerMod.LOGGER.debug("计算拖放目标索引: mouseY={}, index={}",
                mouseY, index);
            
            return index;
        } catch (Exception e) {
            MasterPlannerMod.LOGGER.error("计算拖放索引时发生错误: {}", e.getMessage(), e);
            return -1;
        }
    }

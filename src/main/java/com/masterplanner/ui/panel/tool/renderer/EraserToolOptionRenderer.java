...
import com.masterplanner.ui.tools.impl.modify.EraserTool;
import imgui.ImGui;
...
public class EraserToolOptionRenderer extends AbstractToolOptionRenderer {
        ...
        try {
            ...
            if (ImGui.sliderFloat("##eraser_radius", eraserRadius, MIN_ERASER_RADIUS, MAX_ERASER_RADIUS, "%.1f")) {
                ...
                if (eraserTool != null) {
                    ...
                    MasterPlannerMod.LOGGER.info("橡皮擦半径已更新为: {}", eraserRadius[0]);
                }
            }
            ImGui.popItemWidth();
    ...
    public void initialize() {
        ...
        if (eraserTool != null) {
            eraserRadius[0] = eraserTool.getEraserRadius();
            MasterPlannerMod.LOGGER.info("初始化橡皮擦半径: {}", eraserRadius[0]);
        } else {
            eraserRadius[0] = DEFAULT_ERASER_RADIUS;
            MasterPlannerMod.LOGGER.warn("无法获取橡皮擦工具实例，使用默认半径: {}", DEFAULT_ERASER_RADIUS);
        }
    }

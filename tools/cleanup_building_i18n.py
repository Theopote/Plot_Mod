...
unused_path = ROOT / "tools" / "building_i18n_unused.txt"
...
for relative in (
    ...
    remaining = sum(
        1
        for key in data
        if key.startswith("plugin.building.") or key.startswith("hint.plot.building.")
    )
    print(f"{relative}: removed {removed}, remaining building keys={remaining}")
